...
const char* conf_common_dataset(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_DATASET, DEF_COMM_DATASET);
}
...
const char* conf_common_data_dir(conf_t* conf) {
	const char* trace_dir = conf_common_trace_dir(conf);
	const char* dataset = conf_common_dataset(conf);
	char *tmp = (char *) malloc(100);
	sprintf(tmp, "%s/%s/", trace_dir, dataset);
	return tmp;
}

const char* conf_common_save_dir(conf_t* conf) {
	const char *data_dir = conf_common_data_dir(conf);
	char *tmp = (char *) malloc(100);
	if (TIME_INTERVAL*1000 >= 1) {
		sprintf(tmp, "%s/%dms/", data_dir, int(TIME_INTERVAL*1000));
	}
	else {
		sprintf(tmp, "%s/%dus/", data_dir, int(TIME_INTERVAL*1000*1000));
	}
	return tmp;
}
...
const char* conf_common_packet_stat_file(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_PACKET_STAT_FILE, DEF_COMM_PACKET_STAT_FILE);
}

const char* conf_common_subset_mapping_file(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_SUBSET_MAPPING_FILE, DEF_COMM_SUBSET_MAPPING_FILE);
}

const char* conf_common_host_mapping_file(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_HOST_MAPPING_FILE, DEF_COMM_HOST_MAPPING_FILE);
}

const char* conf_common_app_mapping_file(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_APP_MAPPING_FILE, DEF_COMM_APP_MAPPING_FILE);
}

const char* conf_common_flow_record_file(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_FLOW_RECORD_FILE, DEF_COMM_FLOW_RECORD_FILE);
}

const char* conf_common_traffic_matrix_file(conf_t* conf) {
	return getstring(conf->dict, SEC_COMM, KEY_COMM_TRAFFIC_MATRIX_FILE, DEF_COMM_TRAFFIC_MATRIX_FILE);
}

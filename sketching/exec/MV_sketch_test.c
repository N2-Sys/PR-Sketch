...
int main(int argc, char* argv[]){
	...
	if (DEBUG) {
		total_bytes = height * 1 * 16;
		sketch = new MVSketch(total_bytes, height);
		debug(sketch);
		delete sketch;
		exit(0);
	}
	...
	sketch = new MVSketch(total_bytes, height);
	next_sketch = new MVSketch(total_bytes, height);

	LOG_MSG("\nTest with CAIDA...\n");
	test();
	delete sketch;
	sketch = new MVSketch(total_bytes, height); // new sketch

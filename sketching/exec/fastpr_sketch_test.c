...
#include "../algorithm/fastpr_sketch.h"
...
int main(int argc, char* argv[]){
	...
	if (DEBUG) {
		sketch = new CBFLegitimateBF(bf_space, measure_space, bf_hash_num, cbf_hash_num, threshold);
		debug(sketch);
		delete sketch;
		exit(0);
	}
	...
	sketch = new CBFLegitimateBF(bf_space, measure_space, bf_hash_num, cbf_hash_num, threshold);
	next_sketch = new CBFLegitimateBF(bf_space, measure_space, bf_hash_num, cbf_hash_num, threshold);

	pktdrop_sketch = new CBFLegitimateBF(bf_space, measure_space, bf_hash_num, cbf_hash_num, threshold);

...
CountHeap::CountHeap(uint32_t total_bytes, uint32_t height, uint32_t heapsize, uint32_t layer_index) {
...
CountHeap::CountHeap(const CountHeap& other) {
...
void CountHeap::update(tuple_key_t cur_key) {
	update(cur_key, false, 0, 1);
}

void CountHeap::update(tuple_key_t cur_key, bool is_ns, uint32_t ns_row, uint32_t ns_value) {
	// Update count sketch
	int32_t tmp_estimation = 0;
	for (uint32_t row_idx = 0; row_idx < height; row_idx++) {
		if (is_ns && row_idx != ns_row) {
			continue;
		}

		hash_result_t hash_result = myhash(cur_key, row_idx, width, layer_index);
		int32_t *cur_bucket = get_bucket(row_idx, hash_result.hash_idx);

		// Count Sketch
		/*if (hash_result.hash_val > 0) {
			(*cur_bucket) += 1;
		}
		else {
			(*cur_bucket) += -1;
		...
		// CM sketch
		(*cur_bucket) += int32_t(ns_value);

		if ((row_idx == 0) || ((*cur_bucket) < tmp_estimation)) {
			tmp_estimation = (*cur_bucket);
		}
	}

	uint32_t estimation = 0;
	if (tmp_estimation > 0) {
		estimation = tmp_estimation;
	}

	// Update heap (hash table)
	uint32_t heap_idx = myhash(cur_key, layer_index, heapsize).hash_idx;
...
uint32_t CountHeap::estimate(tuple_key_t cur_key) {
	int32_t result = 0;
...
set<tuple_key_t> CountHeap::get_tuplekeys() {
...
uint32_t CountHeap::get_transmission_pkts() {
...
uint32_t CountHeap::get_transmission_bytes() {
	uint32_t nbucket = buckets.size();
	uint32_t nbytes = nbucket*4 + heap_vector.size()*12;
	return nbytes;
}

int32_t* CountHeap::get_bucket(uint32_t row, uint32_t col) {
	afs_assert(row >= 0 && row < height, "Invalid row: %d (%d)\n", row, height);
	afs_assert(col >= 0 && col < width, "Invalid col: %d (%d)\n", col, width);
	return &buckets[row*width + col];
}

vector< std::pair<tuple_key_t, uint32_t> > CountHeap::get_heap() {
